...
//reference: https://github.com/bubblebooy/ArtifactDeckCode
...
const CArtifactDeckDecoder = (() => {
    ...
    const DecodeDeckString = $strDeckCode => {
        ...
        let $decoded = decode($strNoPrefix); //base64_decode( $strNoPrefix );
        //console.log($decoded)
        let $deckBytes = [];
    ...
    const ParseDeckInternal = ($strDeckCode, $deckBytes) => {
        ...
        if ($nCurrentByteIndex[0] <= $nTotalBytes) {
            ...
            $name = $bytes.map(b => String.fromCharCode(b)).join('');
